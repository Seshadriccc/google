...
import { CheckCircle, Clock, User, Eye, MessageSquare, AlertTriangle, X } from 'lucide-react';
...
export default function StaffDash() {
    ...
    useEffect(() => {
        ...
        const q = query(
            ...
            where("status", "!=", "Resolved"), // Removed category filter to avoid missing index
            orderBy("status"),
            orderBy("createdAt", "desc")
        );
        const unsubscribe = onSnapshot(q, (snapshot) => {
            const allIssues = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
            // Client-side filter for Academics
            setIssues(allIssues.filter(i => i.category === "Academics"));
        });
        return () => unsubscribe();
    }, []);
        ...
        <div className="flex h-[calc(100vh-80px)] overflow-hidden">
            ...
            <div className="w-1/3 bg-white border-r border-slate-200 overflow-y-auto">
                <div className="p-4 border-b border-slate-100 bg-slate-50 sticky top-0 z-10">
                    <h2 className="font-bold text-slate-700">Academic Issues ({issues.length})</h2>
                </div>
                {issues.map(issue => (
                    <div
                        key={issue.id}
                        onClick={() => setSelectedIssue(issue)}
                        className={`p-4 border-b border-slate-100 cursor-pointer hover:bg-purple-50 transition-colors ${selectedIssue?.id === issue.id ? 'bg-purple-50 border-l-4 border-purple-500' : ''}`}
                    >
            ...
            <div className="w-2/3 bg-slate-50 p-8 overflow-y-auto relative">
                {selectedIssue ? (
                    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8 max-w-2xl mx-auto">
                            ...
                            <div>
                                ...
                                <div className="flex gap-2">
                                    ...
                                    {selectedIssue.evidenceUrl && (
                                        <a href={selectedIssue.evidenceUrl} target="_blank" rel="noreferrer" className="flex items-center gap-1 bg-purple-100 px-3 py-1 rounded-full text-xs font-bold text-purple-600 hover:bg-purple-200">
                        ...
                        {/* Sanitized Content */}
                        <div className="bg-purple-50 p-6 rounded-xl border border-purple-100 mb-8 relative group">
                            <div className="flex justify-between items-center mb-2">
                                <h3 className="text-xs font-bold text-purple-400 uppercase">Description</h3>
                                {selectedIssue.originalText && (
                                    <span className="text-[10px] bg-purple-100 text-purple-600 px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            {selectedIssue.originalText && selectedIssue.originalText !== selectedIssue.text && (
                                <div className="mt-4 pt-4 border-t border-purple-200/50 opacity-40 hover:opacity-100 transition-opacity">
                                    <h4 className="text-[10px] font-bold text-slate-400 uppercase mb-1">Original Submission</h4>
                            ...
                            <div className="border-t border-slate-100 pt-8">
                                        ...
                                        <div className="space-y-3">
                                            {selectedIssue.updates.map((update, idx) => (
                                                <div key={idx} className="flex gap-3 text-sm">
                                                    <div className="w-1 bg-purple-300 rounded-full"></div>
                                                    <div>
                                ...
                                <div className="flex gap-4">
                                    <button
                                        ...
                                        className="px-4 bg-white border-2 border-slate-200 text-slate-700 font-bold py-3 rounded-xl hover:border-purple-500 hover:text-purple-500 transition-all flex justify-center items-center gap-2"
                ...
                {/* Modals - Same logic, different titles/colors if desired (Keeping generic) */}
                    ...
                    <div className="absolute inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center z-50 p-8">
                        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 animate-in fade-in zoom-in duration-200">
                            ...
                            <textarea
                                ...
                                placeholder="Why escalate? (e.g. Beyond my authority, needs HoD approval)"
                            />
                ...
                {isUpdateModalOpen && (
                    <div className="absolute inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center z-50 p-8">
                        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
                            <h3 className="text-lg font-bold text-slate-800 mb-4">Post Update</h3>
                            <textarea value={updateNote} onChange={e => setUpdateNote(e.target.value)} className="w-full h-32 p-3 border rounded-xl mb-4" placeholder="Update details..."></textarea>
                            <button onClick={confirmUpdate} className="w-full bg-slate-800 text-white py-3 rounded-xl font-bold">Post Update</button>
                            <button onClick={() => setIsUpdateModalOpen(false)} className="w-full mt-2 text-slate-500 py-2">Cancel</button>
                        </div>
                    </div>
                )}
                ...
                {isResolveModalOpen && (
                    <div className="absolute inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center z-50 p-8">
                        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
                            <h3 className="text-lg font-bold text-green-600 mb-4">Confirm Resolution</h3>
                            <textarea value={reason} onChange={e => setReason(e.target.value)} className="w-full h-32 p-3 border rounded-xl mb-4" placeholder="Resolution notes..."></textarea>
                            <button onClick={confirmResolution} className="w-full bg-green-600 text-white py-3 rounded-xl font-bold">Resolve</button>
                            <button onClick={() => setIsResolveModalOpen(false)} className="w-full mt-2 text-slate-500 py-2">Cancel</button>
                        </div>
                    </div>
                )}
            </div>
